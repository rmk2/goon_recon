...
(define (sql-super-create-latest-pseudomaterialized-view)
  (if (table-exists? sqlc "intelSuperLatestMV")
      ...
      (query-exec sqlc (string-append
			...
			"AS SELECT "
			"ship.TypeID AS shipTypeID,ship.typeName AS shipTypeName,mv.characterID,mv.characterName,"
			"mv.corporationID,corporation.corporationTicker,mv.corporationName,"
			"mv.allianceID,alliance.allianceTicker,mv.allianceName,mv.eventType,mv.killID,"
			...
			"constellation.constellationName,region.regionID,region.regionName,mv.datetime,"
			"mv.killCount,mv.activityAvg,mv.activityStd "
			"FROM ( "
			"SELECT "
			...
			"FROM intelSuperRaw GROUP BY characterID DESC "
			") AS mv "
